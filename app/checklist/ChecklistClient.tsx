...
export default function ChecklistClient({ dueItems, userId, frequencyLabels, roleLabels }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (logsToUpsert.length > 0) {
      // UPSERT с conflict resolution — защита от дублей при двойном сабмите (BUG-04)
      ...
      if (error) {
        ...
        setSubmitError('Не удалось сохранить: ' + error.message)
        setSaving(false)
        return
      }
    }
                ...
                <div key={task.id} className={`px-4 py-3 ${decision === 'done' ? 'bg-green-50/50' : decision === 'skipped' ? 'bg-gray-50' : ''}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Примечание при пропуске — с label (UX-09) */}
                      {decision === 'skipped' && (
                        <div className="mt-2">
                          <label
